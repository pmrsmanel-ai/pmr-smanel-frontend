...
export default function LaporanPage() {
  ...
  const denda = useMemo(() => (data?.denda || []).filter(x => inPeriod(x.Tanggal_Terbit, tanggalMulai, tanggalAkhir)).sort((a,b) => tanggalKey(b.Tanggal_Terbit).localeCompare(tanggalKey(a.Tanggal_Terbit))), [data?.denda, tanggalMulai, tanggalAkhir]);
  ...
  function renderKeuangan() {
    return <>
      ...
      <section className="report-source-grid-v3">
        {['KAS','DENDA'].map(source => <div key={source} className={`report-source-card-v3 ${source.toLowerCase()}`}><span>{source}</span><div><small>Pemasukan</small><strong>{rupiah(ringkasan[source]?.pemasukan)}</strong></div><div><small>Pengeluaran</small><strong>{rupiah(ringkasan[source]?.pengeluaran)}</strong></div><div className="source-total"><small>Saldo</small><strong>{rupiah(ringkasan[source]?.saldo)}</strong></div></div>)}
      </section>
  ...
  function renderDenda() {
    return <>
      ...
      <section className="report-table-card-v3"><div className="report-table-head-v3"><div><span>REKAP DENDA</span><h2>Daftar Denda Anggota</h2></div><small>{denda.length} denda</small></div><div className="report-table-wrap-v3"><table><thead><tr><th>Tanggal</th><th>Denda</th><th>ID Anggota</th><th>Nama</th><th>Jenis</th><th>Nominal</th><th>Status</th></tr></thead><tbody>{denda.map((x,i)=>{const member=anggotaMap[String(x.ID_Anggota||'').toUpperCase()]; const paid=String(x.Status||'').toUpperCase()==='LUNAS'; return <tr key={x.Denda_ID||i}><td>{formatTanggal(x.Tanggal_Terbit)}</td><td>{x.Denda_ID||'-'}</td><td>{x.ID_Anggota||'-'}</td><td>{member?.Nama_Lengkap||'-'}</td><td>{x.Jenis_Denda||'-'}</td><td className="money-cell">{rupiah(x.Nominal)}</td><td><Badge tone={paid?'lunas':'belum'}>{x.Status||'-'}</Badge></td></tr>})}{!denda.length&&<tr><td colSpan="7"><div className="report-empty-v3">Belum ada denda pada periode ini.</div></td></tr>}</tbody></table></div></section>
    </>;
  }
